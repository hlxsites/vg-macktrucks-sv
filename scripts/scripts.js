...
export function decorateLinks(block) {
  [...block.querySelectorAll('a')]
    ...
    .forEach((link) => {
      ...
      const external = !url.host.match('sv.macktrucks.com') && !url.host.match('.hlx.(page|live)') && !url.host.match('localhost');
      if (url.host.match('build.sv.macktrucks.com') || url.pathname.endsWith('.pdf') || external) {
        link.target = '_blank';
      }
    });
}
